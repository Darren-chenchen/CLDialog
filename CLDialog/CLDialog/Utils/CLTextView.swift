...
typealias CLTextViewTextDidChangeClouse = (CGFloat) -> ()
...
class CLTextView: UITextView {
    ...
    var textChangeClouse: CLTextViewTextDidChangeClouse?
    ...
    func setupTexiView(){
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(self.textDidChange(notic:)), name: NSNotification.Name.UITextViewTextDidChange, object: self)
        
        self.maxLength = CLDialogManager.share.maxLength
        self.onlyNumberAndPoint = CLDialogManager.share.onlyNumberAndPoint
        self.onlyNumber = CLDialogManager.share.onlyNumber
        self.allowEmoji = CLDialogManager.share.allowEmoji
        self.pointLength = CLDialogManager.share.pointLength
        self.predicateString = CLDialogManager.share.predicateString
    }
    //MARK - 监听文字改变
    @objc func textDidChange(notic: Notification){
        ...
        if self.maxLength != nil && self.maxLength! > 0 {
            if (self.text.count) > (self.maxLength)! {
                let subString = self.text.mySubString(to: (self.maxLength)!)
                self.text = subString
            }
        }
    ...
    func dealPredicate() {
        ...
        var newInputStr = self.text.mySubString(from: self.tempText?.count ?? 0)
        ...
        if self.pointLength != nil && self.pointLength! > 0 {
            ...
            if arr?.count == 2 { // 有小数点
                ...
                if pointConut >= self.pointLength! {
                    self.tempText = (arr?.first ?? "") + "." + (arr?.last?.mySubString(to: self.pointLength!) ?? "")
                    newInputStr = ""
                } else {
                    let count = (arr?.last?.count ?? 0) + newInputStr.count
                    if count > self.pointLength! {
                        let newCount = count - self.pointLength!
                        newInputStr = newInputStr.mySubString(to: newCount)
                    }
                }
            }
        }
